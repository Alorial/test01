...
def Stupid_Human(X, Y, K, color, armsTF):
	face = Circle(Point(X, Y), 20*K)
	face.setFill(color)
	face.setOutline(color)
	body = Line(Point(X, Y+20*K), Point(X, Y+60*K))
	body.setOutline(color)
	leg1 = Line(Point(X, Y+60*K), Point(X+20*K, Y+90*K))
	leg1.setOutline(color)
	leg2 = Line(Point(X, Y+60*K), Point(X-20*K, Y+90*K))
	leg2.setOutline(color)
	arm1 = Line(Point(X, Y+20*K), Point(X+20*K, Y+40*K))
	arm1.setOutline(color)
	arm2 = Line(Point(X, Y+20*K), Point(X-20*K, Y+40*K))
...
Stupid_Human (850, 500, 1, "black", armsTF=True)
